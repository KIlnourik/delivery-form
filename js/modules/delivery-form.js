...
const phoneInputWrapper = deliveryBlock.querySelector('.phone');
...
const getInvalidInputs = () => {
  const invalidInputs = [];
  if (!validateByRegExp(ADDRESS_REGEXP, addressInput.value, addressInputWrapper)) { invalidInputs.push(addressInput.id); }
